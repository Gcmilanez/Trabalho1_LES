...
int main(int argc, char** argv) {
    ...
    std::cout << "[BST-Iter] Arquivo: " << data_file << " | buscas: " << num_searches << std::endl;

    auto data = load_tree_data(data_file);
    if (data.empty()) {
        std::cerr << "[BST-Iter] Dataset vazio ou não encontrado.\n";
        return 1;
    }
    ...
    std::cout << "[BST-Iter] Tempo inserção (ms): " << insert_ms << "\n";
    std::cout << "[BST-Iter] Tempo busca    (ms): " << search_ms << "\n";
    std::cout << "[BST-Iter] found_count: " << found_count << std::endl;
    return 0;
}
